...
function ensureAuthConsent() {
  ...
  if (!submitBtn || document.getElementById("authConsentWrap")) return;

  const wrap = document.createElement("div");
  wrap.id = "authConsentWrap";
  wrap.className = "auth-only-signup hidden";
  wrap.style.cssText = "margin:14px 0 16px;text-align:left;color:#cfc4dc;font-size:13px;line-height:1.45;";
  wrap.innerHTML = `
    <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;">
      <input id="authConsent" type="checkbox" style="width:20px;height:20px;min-width:20px;margin-top:1px;accent-color:#d62cff;">
      <span>
        I agree to the
        <a href="terms.html" target="_blank" rel="noopener" style="color:#d9a7ff;text-decoration:underline;">Terms &amp; Conditions</a>
        and
        <a href="privacy.html" target="_blank" rel="noopener" style="color:#d9a7ff;text-decoration:underline;">Privacy Policy</a>,
        and consent to BeatTag collecting and using my information as described in the Privacy Policy.
      </span>
    </label>`;
...
async function submitAuth() {
  ...
  try {
    if (authMode === "signup") {
      ...
      if (!consent?.checked) {
        message.textContent = "Please agree to the Terms & Conditions and Privacy Policy to create your account.";
        return;
      }
...
async function btMusicSearchNow() {
  ...
  // Then search YouTube securely through the Supabase Edge Function.
  // The YouTube API key stays in Supabase Secrets and is never shipped in app.js.
  try {
