...
function openNotificationFilters(){modal.classList.remove('hidden');modalCard.innerHTML=`<div class="modal-head"><h3>Notification Filters</h3><button class="close" onclick="closeModal()">×</button></div><p class="muted">Choose which activity you want to focus on.</p>${['All','Likes','Comments','Tags','Follows','Messages'].map(x=>`<button class="secondary" style="width:100%;margin:5px 0" onclick="btSet('bt_notif_filter','${x.toLowerCase()}');toast('${x} filter selected.')">${x}</button>`).join('')}`}
