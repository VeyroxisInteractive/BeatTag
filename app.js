...
function renderFeed(
  ...
  arr.forEach((c, index) => {
    ...
    // Reserve one real Android Native Ad position inside the scrolling feed.
    // The Android NativeAdView is moved over this slot while the page scrolls.
    if (index === 1) {
      const adSlot = document.createElement('div');
      adSlot.id = 'btNativeAdSlot';
      adSlot.setAttribute('aria-hidden', 'true');
      adSlot.style.width = '100%';
      adSlot.style.height = '300px';
      adSlot.style.margin = '14px 0';
      adSlot.style.pointerEvents = 'none';
...
function setBeatTagBannerVisible(visible){
  try {
    const allowed = !!visible && currentTab === 'home';
    ...
    if (allowed) requestAnimationFrame(updateBeatTagNativeAdPosition);
  } catch (_) {}
}

let btNativeAdPositionFrame = 0;
function updateBeatTagNativeAdPosition(){
  try {
    if (btNativeAdPositionFrame) cancelAnimationFrame(btNativeAdPositionFrame);
    btNativeAdPositionFrame = requestAnimationFrame(() => {
      btNativeAdPositionFrame = 0;
      const slot = document.getElementById('btNativeAdSlot');
      const bridge = window.BeatTagAndroid;
      if (!bridge || typeof bridge.setNativeAdPosition !== 'function') return;
      if (currentTab !== 'home' || !slot) {
      ...
      bridge.setNativeAdPosition(Math.round(rect.top * dpr), visible);
    });
  } catch (_) {}
}
