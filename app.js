...
function renderMessageBubble(m,all){
  ...
  const media=m.media_url ? (m.message_type==='image'?`<img class="chat-media" src="${escapeAttr(m.media_url)}" alt="">`:m.message_type==='video'?`<video class="chat-media" src="${escapeAttr(m.media_url)}" controls playsinline></video>`:`<audio class="chat-audio" src="${escapeAttr(m.media_url)}" controls></audio>`) : '';
