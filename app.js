...
async function btYouTubeInvoke(query, attempts = 3) {
  let lastError = null;
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    if (attempt < attempts - 1) {
      await btYouTubeDelay(500 * Math.pow(2, attempt));
    }
  }
...
async function btMusicSearchNow() {
  ...
  // Keep the existing BeatTag creator-music search exactly as before.
  await btMusicLoad(query);
  ...
  // If this exact query worked before, keep those results visible while
  // a fresh server request is made. A temporary network/API hiccup should
  // not blank a previously working search.
  if (cached?.length) {
    btMusicRenderYouTube(cached);
  }

  try {
    const data = await btYouTubeInvoke(query, 3);
    ...
    if (youtubeTracks.length) {
      btYouTubeLastGood.set(cacheKey, youtubeTracks);

      // Remove cached YouTube rows before painting the fresh response.
      const currentList = $('#btMusicList');
      currentList?.querySelectorAll('.music-source-heading,.music-track[data-bt-youtube="1"]')
        .forEach(el => el.remove());

      btMusicRenderYouTube(youtubeTracks);
    } else if (!cached?.length) {
      toast('No YouTube results found.');
    }
  } catch (err) {
    console.error('YouTube search failed after retry:', err);

    if (requestId !== btYouTubeSearchSeq) return;

    if (cached?.length) {
      // Keep the last successful results on screen. No scary failure toast
      // for a short-lived connection/API interruption.
      return;
    }

    toast(
      navigator.onLine
        ? 'YouTube is temporarily unavailable. Try Search again.'
