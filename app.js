...
async function renderMessagesInbox(){
  currentTab='messages'; stopStream(); activeConversationId=null;
...
async function openConversation(conversationId){
  activeConversationId=conversationId; currentTab='messages'; stopStream();
  screenEl.innerHTML=`<section class="chat-shell"><div class="chat-head"><button class="ghost" onclick="renderMessagesInbox()">←</button><div id="chatPerson"><strong>Chat</strong></div><button class="ghost" onclick="openChatMenu()">•••</button></div><div id="chatMessages" class="chat-messages"><div class="empty">Loading...</div></div><div id="replyBar" class="reply-bar hidden"></div><div class="chat-compose"><button class="chat-attach" onclick="document.getElementById('chatFileInput').click()">＋</button><input id="chatFileInput" type="file" accept="image/*,video/*,audio/*" hidden onchange="sendChatAttachment(this.files[0]);this.value=''"><textarea id="chatInput" rows="1" maxlength="4000" placeholder="Type a message..."></textarea><button class="chat-send" onclick="sendChatMessage()">➤</button></div></section>`;
...
async function loadConversationMessages(scroll=true){
  ...
  try{
    ...
    const other=(parts||[]).find(x=>x.user_id!==currentUserId); const cp=document.getElementById('chatPerson'); if(cp&&other?.profiles)cp.innerHTML=`<strong>${escapeHTML(other.profiles.name||'BeatTag User')}</strong><small>@${escapeHTML(other.profiles.username||'user')}</small>`;
...
async function sendChatMessage(){const input=document.getElementById('chatInput');const body=input?.value.trim();if(!body||!activeConversationId)return;input.disabled=true;try{const {error}=await supabaseClient.from('messages').insert({conversation_id:activeConversationId,sender_id:currentUserId,body,message_type:'text',reply_to:replyingToMessageId});if(error)throw error;input.value='';cancelMessageReply();await loadConversationMessages();}catch(e){toast(e.message||'Message failed.');}finally{input.disabled=false;input.focus();}}
...
async function openChatMenu(){if(!activeConversationId)return;modal.classList.remove('hidden');modalCard.innerHTML=`<div class="modal-head"><h3>Chat options</h3><button class="close" onclick="closeModal()">×</button></div><div class="bt-option-stack"><button class="secondary" onclick="togglePinActiveChat()">📌 Pin / Unpin chat</button><button class="secondary" onclick="openChatMediaGallery()">🖼 Media gallery</button><button class="secondary" onclick="openChatPollCreator()">📊 Create poll</button><button class="secondary" onclick="openScheduleMessage()">🕒 Schedule message</button><button class="secondary" onclick="blockChatUser()">🚫 Block user</button><button class="secondary" onclick="renderMessagesInbox();closeModal()">Close chat</button></div>`;}
