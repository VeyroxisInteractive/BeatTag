...
async function submitAuth() {
  ...
  try {
    if (authMode === "signup") {
      ...
      const consent = document.getElementById("authConsent");
      if (!consent?.checked) {
        const accepted = await beatConfirm(
          "Consent Required",
          "To create your BeatTag account, please review and accept the Privacy Policy and Terms & Conditions, and consent to the collection and use of your account information.",
          "Review & Accept"
        );

        if (accepted) {
          consent?.scrollIntoView({ behavior: "smooth", block: "center" });
          setTimeout(() => consent?.focus(), 350);
        }

        message.textContent =
          "Please tick the consent checkbox before creating your account.";
        return;
      }
...
function go(tab) {
  ...
  setBeatTagBannerVisible(['home', 'explore', 'featured', 'chains'].includes(tab));
  stopStream();
...
function getBeatTagNativeAdPositions(itemCount) {
  const positions = [2, 6, 10, 12];
  let position = 12;
  let gap = 6;

  // 12 -> 18 (+6) -> 27 (+9) -> 39 (+12) -> 54 (+15) ...
  while (true) {
    position += gap;
...
function createBeatTagNativeAdSlot(section, position) {
  ...
  slot.className = 'bt-native-ad-slot';
...
function renderFeed(
  ...
  arr.forEach((c, index) => {
    ...
    if (shouldInsertBeatTagNativeAd(challengeNumber, arr.length)) {
      feed.appendChild(createBeatTagNativeAdSlot('home', challengeNumber));
    }
  });

  requestAnimationFrame(updateBeatTagNativeAdPosition);
}
...
function renderFeatured() {
  ...
  arr.forEach(
    (c, index) => {
      ...
      const challengeNumber = index + 1;
      if (shouldInsertBeatTagNativeAd(challengeNumber, arr.length)) {
        feed.appendChild(createBeatTagNativeAdSlot('featured', challengeNumber));
      }
    }
  );
...
async function doSearch(typeFilter = null) {
  ...
  if (challengeMatches.length) {
    ...
    challengeMatches.forEach((c, index) => {
      out.appendChild(challengeCard(c));
      const challengeNumber = index + 1;
      if (shouldInsertBeatTagNativeAd(challengeNumber, challengeMatches.length)) {
        out.appendChild(createBeatTagNativeAdSlot('explore', challengeNumber));
      }
    });
    requestAnimationFrame(updateBeatTagNativeAdPosition);
  }
...
function renderChains() {
  ...
  roots
    ...
    .forEach((root, index) => {
      ...
      if (shouldInsertBeatTagNativeAd(chainNumber, roots.length)) {
        tree.appendChild(createBeatTagNativeAdSlot('chains', chainNumber));
      }
    });
...
const BEATTAG_NATIVE_AD_TABS = new Set(['home', 'explore', 'featured', 'chains']);
let btNativeAdFrame = 0;

function setBeatTagBannerVisible(visible) {
  try {
    const allowed = !!visible && BEATTAG_NATIVE_AD_TABS.has(currentTab);
    if (window.BeatTagAndroid && typeof window.BeatTagAndroid.setBannerVisible === 'function') {
      window.BeatTagAndroid.setBannerVisible(allowed);
    }
    requestAnimationFrame(updateBeatTagNativeAdPosition);
  } catch (_) {}
}

function updateBeatTagNativeAdPosition() {
  try {
    if (btNativeAdFrame) cancelAnimationFrame(btNativeAdFrame);

    btNativeAdFrame = requestAnimationFrame(() => {
      btNativeAdFrame = 0;
      const bridge = window.BeatTagAndroid;
      if (!bridge || typeof bridge.setNativeAdPosition !== 'function') return;

      if (!BEATTAG_NATIVE_AD_TABS.has(currentTab) || document.body.classList.contains('bt-chat-open')) {
        bridge.setNativeAdPosition(0, false);
        return;
      }

      const slots = Array.from(document.querySelectorAll('.bt-native-ad-slot'));
      if (!slots.length) {
        bridge.setNativeAdPosition(0, false);
        return;
      }
      ...
      const visibleSlots = slots
        ...
        .filter(item => item.rect.bottom > 0 && item.rect.top < viewportHeight);
      ...
      // Only one Android NativeAdView exists. Move it to the visible feed slot.
      const viewportCenter = viewportHeight / 2;
      visibleSlots.sort((a, b) => {
        const ac = (a.rect.top + a.rect.bottom) / 2;
        const bc = (b.rect.top + b.rect.bottom) / 2;
        return Math.abs(ac - viewportCenter) - Math.abs(bc - viewportCenter);
      });

      const rect = visibleSlots[0].rect;
      bridge.setNativeAdPosition(Math.round(rect.top * dpr), true);
    });
  } catch (_) {}
}

window.addEventListener('scroll', updateBeatTagNativeAdPosition, { passive: true });
window.addEventListener('resize', updateBeatTagNativeAdPosition, { passive: true });
function setChatChrome(active){const nav=document.querySelector('.bottom-nav');if(nav)nav.classList.toggle('bt-chat-hidden',!!active);document.body.classList.toggle('bt-chat-open',!!active);setBeatTagBannerVisible(!active&&BEATTAG_NATIVE_AD_TABS.has(currentTab));}
...
async function renderMusic(query=''){
  screenEl.innerHTML=`<div class="music-shell bt-music-v2">
    <section class="btm-head">
      <div>
        <div class="btm-kicker">BEATTAG MUSIC</div>
        <h1>Music</h1>
        <p>Discover songs, artists and BeatTag creators.</p>
      </div>
      <button class="btm-upload" onclick="openMusicUpload()">＋ Upload</button>
    </section>

    <div class="btm-search">
      <span>⌕</span>
      <input id="btMusicSearch" value="${escapeAttr(query)}" placeholder="Search songs, artists, albums…" autocomplete="off">
      <button type="button" onclick="btMusicSearchNow()">Search</button>
    </div>

    <div class="btm-categories" aria-label="Music categories">
      <button onclick="btMusicQuickSearch('trending music')"><b>🔥</b><small>Trending</small></button>
      <button onclick="btMusicQuickSearch('Hindi songs')"><b>♫</b><small>Hindi</small></button>
      <button onclick="btMusicQuickSearch('Punjabi songs')"><b>◉</b><small>Punjabi</small></button>
      <button onclick="btMusicQuickSearch('English songs')"><b>♬</b><small>English</small></button>
      <button onclick="btMusicShowCreators()"><b>♙</b><small>Creators</small></button>
    </div>

    ${query ? `
      <section class="music-section btm-results-section">
        <div class="btm-section-head"><h2>Search Results</h2><span>${escapeHTML(query)}</span></div>
        <div class="btm-filter-tabs">
          <button class="active">All</button><button>Songs</button><button>Artists</button><button>Albums</button><button>Playlists</button>
        </div>
        <div id="btMusicList"><div class="music-empty">Searching music…</div></div>
      </section>
    ` : `
      <section class="btm-for-you">
        <div class="btm-section-head"><h2>For You</h2><button onclick="btMusicQuickSearch('popular music')">See All</button></div>
        <div class="btm-mix-row">
          <button class="btm-mix-card mix-one" onclick="btMusicQuickSearch('today hits')"><span>Today’s Hits</span><small>Hindi · Punjabi · English</small></button>
          <button class="btm-mix-card mix-two" onclick="btMusicQuickSearch('chill songs')"><span>Chill Vibes</span><small>Relax & unwind</small></button>
          <button class="btm-mix-card mix-three" onclick="btMusicQuickSearch('party songs')"><span>Party Mix</span><small>Turn it up</small></button>
        </div>
      </section>

      <section class="music-section btm-discover-songs">
        <div class="btm-section-head"><h2>Popular Songs</h2><button onclick="btMusicRefreshDiscover()">Refresh</button></div>
        <div id="btDiscoverSongs"><div class="music-empty">Loading songs…</div></div>
      </section>

      <section class="music-section">
        <div class="btm-section-head"><h2>Fresh Uploads</h2><button onclick="renderMusic()">Refresh</button></div>
        <div id="btMusicList"><div class="music-empty">Loading music…</div></div>
      </section>

      <section class="music-section">
        <div class="btm-section-head"><h2>BeatTag Creators</h2><button onclick="btMusicShowCreators()">See All</button></div>
        <button class="btm-creator-card" onclick="btMusicShowCreators()">
          <span class="btm-creator-icon">♫</span>
          <span><strong>Originals</strong><small>Songs from BeatTag artists</small></span>
          <b>→</b>
        </button>
      </section>
    `}
  ...
  $('#btMusicSearch')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      e.preventDefault();
      btMusicSearchNow();
    }
  ...
  if (query) {
    setTimeout(() => btMusicSearchNow(true), 0);
  } else {
    setTimeout(() => btMusicLoadDiscover(false), 0);
  }
}

function btMusicQuickSearch(query){
  const input = $('#btMusicSearch');
  if (input) input.value = query;
  btMusicSearchNow();
}

async function btMusicShowCreators(){
  const input = $('#btMusicSearch');
  if (input) input.value = '';
  await btMusicLoad('');
  btMusicRenderList();
  document.querySelector('.btm-for-you')?.scrollIntoView({behavior:'smooth',block:'end'});
}

let btYouTubeSearchSeq = 0;
let btYouTubeLastGood = new Map();
const BT_YT_CACHE_KEY = 'beattag_youtube_search_cache_v2';
const BT_YT_CACHE_TTL = 12 * 60 * 60 * 1000; // 12 hours
const BT_YT_CACHE_MAX = 60;

function btYouTubeReadCache() {
  try {
    const raw = JSON.parse(localStorage.getItem(BT_YT_CACHE_KEY) || '{}');
    const now = Date.now();
    Object.entries(raw).forEach(([key, entry]) => {
      if (entry && Array.isArray(entry.items) && now - Number(entry.time || 0) < BT_YT_CACHE_TTL) {
        btYouTubeLastGood.set(key, entry.items);
      }
    });
  } catch (_) {}
}

function btYouTubeWriteCache(key, items) {
  try {
    btYouTubeLastGood.set(key, items);
    const all = {};
    [...btYouTubeLastGood.entries()]
      .slice(-BT_YT_CACHE_MAX)
      .forEach(([k, value]) => {
        all[k] = { time: Date.now(), items: value };
      });
    localStorage.setItem(BT_YT_CACHE_KEY, JSON.stringify(all));
  } catch (_) {}
}

btYouTubeReadCache();

function btYouTubeDelay(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

function btYouTubeErrorStatus(error) {
  return Number(
...
function btYouTubeErrorText(error) {
  try {
    return String(
      error?.message ||
      error?.context?.body ||
...
function btYouTubeShouldRetry(error) {
  const status = btYouTubeErrorStatus(error);
  const msg = btYouTubeErrorText(error);

  // Never retry quota/auth/permission failures: retries only waste quota.
  if (
...
async function btYouTubeInvoke(query, attempts = 2) {
  let lastError = null;

  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      const result = await supabaseClient.functions.invoke('bright-action', {
        body: { query }
      });
      ...
      const apiMessage =
        typeof result.data?.error === 'string'
          ? result.data.error
          : (result.data?.error?.message || 'YouTube search failed');

      lastError = result.error || new Error(apiMessage);

      if (!btYouTubeShouldRetry(lastError)) break;
    } catch (err) {
      lastError = err;
      if (!btYouTubeShouldRetry(lastError)) break;
    }
...
const BT_DISCOVER_QUERY = 'Hindi Punjabi English popular songs official music';
const BT_DISCOVER_CACHE_KEY = 'beattag_discover_songs_v1';
const BT_DISCOVER_TTL = 6 * 60 * 60 * 1000;

function btMusicRenderDiscover(tracks) {
  const box = $('#btDiscoverSongs');
  if (!box) return;

  if (!tracks?.length) {
    box.innerHTML = '<div class="music-empty">Songs unavailable right now. Use Search above.</div>';
    return;
  }
  ...
  tracks.slice(0, 12).forEach(track => {
    ...
    row.className = 'music-track btm-discover-track';
    row.innerHTML = `
      ${track.cover
        ? `<img class="music-cover" src="${escapeAttr(track.cover)}" alt="" loading="lazy">`
        : `<div class="music-cover music-cover-fallback">♫</div>`}
      <div class="music-track-info">
        <strong>${escapeHTML(track.title || 'Song')}</strong>
        <small>${escapeHTML(track.artist || 'YouTube')}</small>
      </div>
      <button class="music-play" type="button" aria-label="Play">▶</button>`;
    row.addEventListener('click', () => btPlayYouTube(track.videoId, track.title || 'Song'));
    box.appendChild(row);
  });
}

async function btMusicLoadDiscover(force = false) {
  const box = $('#btDiscoverSongs');
  if (!box) return;

  if (!force) {
    try {
      const saved = JSON.parse(localStorage.getItem(BT_DISCOVER_CACHE_KEY) || 'null');
      if (saved?.items?.length && Date.now() - Number(saved.time || 0) < BT_DISCOVER_TTL) {
        btMusicRenderDiscover(saved.items);
        return;
      }
    } catch (_) {}
  }

  box.innerHTML = '<div class="music-empty">Loading popular songs…</div>';

  try {
    const data = await btYouTubeInvoke(BT_DISCOVER_QUERY, 2);
    const seen = new Set();
    ...
    if (tracks.length) {
      try {
        localStorage.setItem(BT_DISCOVER_CACHE_KEY, JSON.stringify({
          time: Date.now(),
          items: tracks
        }));
      } catch (_) {}
    }
    btMusicRenderDiscover(tracks);
  } catch (err) {
    console.error('BeatTag discover songs failed:', err);
    try {
      const saved = JSON.parse(localStorage.getItem(BT_DISCOVER_CACHE_KEY) || 'null');
    ...
    btMusicRenderDiscover([]);
  }
}

function btMusicRefreshDiscover() {
  try { localStorage.removeItem(BT_DISCOVER_CACHE_KEY); } catch (_) {}
  btMusicLoadDiscover(true);
}

async function btMusicSearchNow(fromRender = false) {
  const input = $('#btMusicSearch');
  ...
  const requestId = ++btYouTubeSearchSeq;
  const list = $('#btMusicList');

  if (list) {
    list.innerHTML = '<div class="music-empty">Searching songs…</div>';
  }

  // Keep BeatTag's own authorized/creator catalog.
  await btMusicLoad(query);
  if (requestId !== btYouTubeSearchSeq) return;
  btMusicRenderList();

  const cacheKey = query.toLowerCase().replace(/\s+/g, ' ').trim();
  const cached = btYouTubeLastGood.get(cacheKey);

  // Cached searches open immediately and do not consume another API search.
  if (cached?.length) {
    btMusicRenderYouTube(cached);
    return;
  }

  try {
    const data = await btYouTubeInvoke(query, 2);
    if (requestId !== btYouTubeSearchSeq) return;
    ...
    if (youtubeTracks.length) {
      btYouTubeWriteCache(cacheKey, youtubeTracks);
      btMusicRenderYouTube(youtubeTracks);
    } else {
      toast('No songs found for this search.');
    }
  } catch (err) {
    console.error('YouTube song search failed:', err);

    if (requestId !== btYouTubeSearchSeq) return;

    const status = btYouTubeErrorStatus(err);
    const msg = btYouTubeErrorText(err);

    if (!navigator.onLine) {
      toast('No internet connection.');
    } else if (status === 403 || msg.includes('quota')) {
      toast('Music search limit reached for now. Try again later.');
    } else {
      toast('Music search is temporarily unavailable. Try again.');
    }
  }
}
