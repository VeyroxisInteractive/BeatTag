...
function renderMusic(){
  ...
  const sections=[
    ['🔥','Trending','What BeatTag creators are playing now'],
    ['🌱','Rising Artists','Discover independent and emerging creators'],
    ['✨','Fresh Uploads','New original and rights-cleared music'],
    ['🎧','For You','Your music discovery will personalize here']
  ];
  screenEl.innerHTML=`
    <section class="music-hero">
      <div class="music-kicker">BEATTAG MUSIC</div>
      <h1>Feel the Beat.<br>Create the Moment.</h1>
      <p>Listen, discover independent artists and use eligible music in BeatTag challenges.</p>
      <div class="music-search"><span>⌕</span><input id="musicSearch" type="search" placeholder="Search songs, artists, genres..." oninput="filterMusicSections(this.value)"></div>
    </section>
    <div class="music-chips">${['For You','Hindi','English','Punjabi','Marathi','Hip-Hop','Pop','Lo-fi'].map(x=>`<button class="pill">${x}</button>`).join('')}</div>
    <section class="music-now-card"><div class="music-cover">♪</div><div><small>BEATTAG MUSIC</small><strong>Ready for your sound</strong><span>Music player + background playback foundation</span></div><button class="music-play" onclick="toast('Choose a track to play.')">▶</button></section>
    <div id="musicDiscovery">${sections.map(([icon,title,sub])=>`<section class="music-section" data-music-search="${(title+' '+sub).toLowerCase()}"><div class="section-title"><h2>${icon} ${title}</h2><span class="muted">See all</span></div><div class="music-empty-card"><div class="music-empty-icon">♫</div><div><strong>${title}</strong><p>${sub}. Tracks will appear here as the BeatTag Music catalog grows.</p></div></div></section>`).join('')}</div>
    <section class="artist-studio-card"><div><span class="music-kicker">FOR CREATORS</span><h2>Upload Your Music</h2><p>Independent artists will be able to publish original or properly licensed tracks with cover art, language, genre and credits.</p></div><button class="primary" onclick="openMusicArtistStudio()">Open Artist Studio</button></section>`;
}
function filterMusicSections(value=''){const q=String(value).trim().toLowerCase();document.querySelectorAll('[data-music-search]').forEach(el=>el.classList.toggle('hidden',!!q&&!el.dataset.musicSearch.includes(q)));}
function openMusicArtistStudio(){modal.classList.remove('hidden');modalCard.innerHTML=`<div class="modal-head"><h3>🎵 Artist Studio</h3><button class="close" onclick="closeModal()">×</button></div><p class="muted">BeatTag Music is prepared for independent creators. Music uploads will require the Music backend tables/storage and rights declaration before publishing is enabled.</p><div class="bt-list-card"><strong>Original / authorized music only</strong><small>Song title · Artist · Cover · Language · Genre · Credits · Rights confirmation</small></div><div class="bt-list-card"><strong>Artist discovery</strong><small>Rising Artists · Fresh Uploads · Local & regional discovery</small></div><div class="bt-list-card"><strong>Challenge integration</strong><small>Eligible tracks can later offer 15 / 30 / 60 second challenge clips.</small></div>`;}
...
function openNotificationItem(id){const n=(state.notifications||[]).find(x=>x._id===id);if(!n)return;n.read=true;save();updateNotificationDot();closeModal();if(n.conversationId){openConversation(n.conversationId);return;}if(n.challengeId){openChallengeById(n.challengeId);return;}if(n.profileId){renderPublicProfile(n.profileId);return;}if(getNotificationKind(n)==='messages'){go('messages');}}
function notificationAllowed(kind){const map={like:'bt_notify_likes',comment:'bt_notify_comments',follow:'bt_notify_follows',tag:'bt_notify_tags',attempt:'bt_notify_attempts',message:'bt_notify_messages'};return settingEnabled(map[kind]||'',true);}
async function addActivityNotification(kind,text,challengeId='',profileId='',conversationId=''){if(!notificationAllowed(kind))return;state.notifications||=[];state.notifications.unshift({kind,text,time:Date.now(),read:false,challengeId,profileId,conversationId,_id:`${Date.now()}-${Math.random().toString(36).slice(2)}`});save();updateNotificationDot();await showBeatTagNotification('BeatTag',text,challengeId);}
...
const beatTagRealtime = supabaseClient
  ...
  .on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},async(payload)=>{const row=payload?.new||{};if(row.sender_id&&row.sender_id!==currentUserId){await updateMessagesBadge();if(!(currentTab==='messages'&&activeConversationId===row.conversation_id)){await addActivityNotification('message','💬 New message received','','',row.conversation_id||'');}}})
  .subscribe((status) => {
