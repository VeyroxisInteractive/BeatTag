...
function startVideoRecord() {
  ...
  try {
    ...
    recorder.onerror = event => {
      ...
      toast('Video recording failed. Please try again.');
    };
    ...
    recorder.onstop = () => {
      ...
      try {
        ...
        if (!chunks.length) {
          stopStream();
          toast('No video was recorded. Please try again.');
          return;
        }

        // Normalize the final MIME type so BeatTag's existing
        // video validation accepts the recorded WebM file.
        ...
        if (!captureBlob || captureBlob.size === 0) {
          stopStream();
          toast('Video is empty. Please record again.');
          return;
        }
        ...
        // Blob URL is much lighter than converting a video to Base64.
        ...
        stopStream();
        toast('Could not process the video.');
      }
    };

    // Emit chunks every second for better Android WebView reliability.
    recorder.start(1000);
...
function validateMediaFile(file, kind=currentType) {
  ...
  if (!rule.types.includes(type)) return { ok:false, message:`Unsupported ${kind} format. Choose a supported file.` };
...
function openNotifications() {
  ...
  modalCard.innerHTML = `
    ...
    <div class="notification-footer">
      <button class="secondary" onclick="requestBeatTagNotifications()">Notification Settings</button>
    </div>
  `;
