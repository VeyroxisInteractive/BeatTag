...
// Android Native Ad: show only after the user scrolls on supported tabs.
const BT_NATIVE_AD_TABS = new Set(['home', 'explore', 'featured', 'chains']);
let btNativeAdVisible = false;
let btNativeAdHideTimer = null;
let btNativeAdScrollTicking = false;

function btSetNativeAdVisible(visible) {
  const next = !!visible;
  if (btNativeAdVisible === next) return;
  btNativeAdVisible = next;
  setBeatTagBannerVisible(next);
}

function btScheduleNativeAdHide() {
  clearTimeout(btNativeAdHideTimer);
  btNativeAdHideTimer = setTimeout(() => btSetNativeAdVisible(false), 4500);
}

window.addEventListener('scroll', () => {
  if (btNativeAdScrollTicking) return;
  btNativeAdScrollTicking = true;
  requestAnimationFrame(() => {
    btNativeAdScrollTicking = false;
    if (!BT_NATIVE_AD_TABS.has(currentTab)) {
      btSetNativeAdVisible(false);
      return;
    }
    const y = window.scrollY || document.documentElement.scrollTop || document.body.scrollTop || 0;
    if (y > 420) {
      btSetNativeAdVisible(true);
      btScheduleNativeAdHide();
    } else {
      btSetNativeAdVisible(false);
    }
  });
