...
function renderFeed(
  ...
  arr.forEach((c, index) => {
    ...
    // Native ad slots: after challenge 2, then after every 4 challenges.
    const challengeNumber = index + 1;
    if (
      challengeNumber === 2 ||
      (challengeNumber > 2 && (challengeNumber - 2) % 4 === 0)
    ) {
      const slot = document.createElement('div');
      slot.className = 'bt-native-ad-slot';
      slot.dataset.nativeAdSlot = String(challengeNumber);
...
function setBeatTagBannerVisible(visible){/* Inline native ads are positioned by setupBeatTagInlineNativeAds(). */}
...
/* =========================================================
   BEATTAG INLINE ANDROID NATIVE ADS
   Visual slots live inside the WebView feed. The real AdMob
   NativeAdView is positioned over the currently visible slot.
========================================================= */
(function installBeatTagInlineNativeAds(){
  if (document.getElementById('bt-inline-native-ad-style')) return;

  const style = document.createElement('style');
  style.id = 'bt-inline-native-ad-style';
  style.textContent = `
    .bt-native-ad-slot{
      display:block;
      width:calc(100% - 24px);
...
let btNativeAdFramePending = false;

function setupBeatTagInlineNativeAds(){
  if (window.__btNativeAdListenersInstalled !== true) {
    window.__btNativeAdListenersInstalled = true;

    const schedule = () => {
      if (btNativeAdFramePending) return;
      btNativeAdFramePending = true;
      requestAnimationFrame(() => {
        btNativeAdFramePending = false;
        syncBeatTagInlineNativeAd();
      });
    };
  ...
  requestAnimationFrame(syncBeatTagInlineNativeAd);
}

function syncBeatTagInlineNativeAd(){
  try{
    if(
      !window.BeatTagAndroid ||
      typeof window.BeatTagAndroid.updateNativeAdSlot !== 'function'
    ) return;

    const slots = Array.from(
      document.querySelectorAll('.bt-native-ad-slot')
    );
    ...
    if(!active){
      window.BeatTagAndroid.updateNativeAdSlot(
        0, 0, 0, 0, false, window.devicePixelRatio || 1
      );
      return;
    }
    ...
    console.warn('BeatTag inline native ad sync failed', e);
  }
}
